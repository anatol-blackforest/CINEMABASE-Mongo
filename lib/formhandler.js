...
module.exports = (err, req, res, typeOfEdit) => {
	...
	if (err) return list((err, films) => render(req, res, films, messages.bigImage));
	//если загруженный файл - НЕ рисунок
	if (req.file && req.file.mimetype && req.file.mimetype.indexOf('image') === -1) return list((err, films) => render(req, res, films));
